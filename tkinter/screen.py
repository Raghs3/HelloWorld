...
mainWindow.columnconfigure(2, weight=1000)  # window is resized then column with weight 3 will widen 3x
...
rbValue = tkinter.IntVar()  # all radio buttons share the same variable as only one can be selected at one time, so previous one is unselected
rbValue.set(2)  # number is the default option
# Radio buttons
...
# Date spinners
daySpin = tkinter.Spinbox(dateFrame, width=3, from_=1, to=31)
