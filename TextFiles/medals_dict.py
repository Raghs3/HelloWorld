...
with open(filename, 'w', encoding='utf-8', newline='') as output_file:
    writer = csv.DictWriter(output_file, fieldnames=columns, extrasaction='ignore', quoting=csv.QUOTE_NONNUMERIC)  # to exclude keys, extrasaction used to avoid error being thrown
    writer.writeheader() # for getting headings in the CSV files
    # for row in medals_table:  # iterate over each row
    #     writer.writerow(row)
    # writer.writerows(medals_table)
    # writer.writerows(sorted(medals_table))  # dicts cant be compared, sorted has to compare
